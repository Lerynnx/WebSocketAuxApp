...
/**
 * Controlador REST que expone un endpoint para enviar transferencias mediante HTTP POST.
 * Además publica notificaciones en el topic WebSocket para que clientes conectados reciban el evento.
 * 
...
public class TransaccionRestController {
    ...
    /**
     * Recibe una transferencia en formato JSON por REST y publica una notificación en el topic WebSocket.
     * Valida el payload y devuelve mensajes de error.
     * 
    ...
    public ResponseEntity<?> transferViaRest(@RequestBody JsonNode node) {
        ...
        if (cantidadNode == null || emisorNode == null || receptorNode == null) {
            return ResponseEntity.badRequest().body(new MensajeRespuesta("Payload debe contener cantidad, numero_cuenta_emisor y numero_cuenta_receptor"));
        }
        ...
        if (!emisorText.matches("^[a-zA-Z0-9]+$") || !receptorText.matches("^[a-zA-Z0-9]+$")) {
            return ResponseEntity.badRequest().body(new MensajeRespuesta("Campo numero_cuenta_emisor o numero_cuenta_receptor con caracteres inválidos (solo letras y números)"));
        }
        ...
        System.out.println("[REST] Recibida transaccion: id=" + t.getId() + ", emisor=" + t.getCuenta_emisor_id() + ", receptor=" + t.getCuenta_receptor_id() + ", cantidad=" + t.getCantidad());

        // Publica al topic WebSocket para que los clientes conectados reciban la notificación
        TransaccionMensajeRespuesta notification = new TransaccionMensajeRespuesta(t.getId(), emisorText, receptorText, t.getCantidad(), t.getFecha_creacion(), t.getEstado_id());
